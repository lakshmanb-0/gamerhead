    ...
    <main>
      <header className="relative text-white">
        <div className="hidden lg:block lg:h-screen xl:h-full relative w-full ">
          <ImageBox realImage={gameData?.screenshots?.[0]?.path_full} customStyle={'z-0 w-full h-full'} noBlurred={false} />
            ...
            <div className="px-5 py-6">
              ...
              <div className="flex items-center gap-2 pt-4 justify-between">
                {!gameData?.release_date?.coming_soon && (
                  (state?.cartData?.includes(gameData?.steam_appid) || added.cart) ?
                ...
                {
                  (state?.wishlistData?.includes(gameData?.steam_appid) || added.wishlist) ?
      ...
      {gameData?.screenshots &&
        <section className="py-5 sm:py-10 maxWidth">
          ...
          <section className="flex gap-4 overflow-auto py-5">
            {gameData?.screenshots?.map((item) => (
              <a className="min-w-[300px]" href={item.path_full} target="_blank" key={item?.id}>
                <ImageBox realImage={item?.path_thumbnail} />
              </a>
            ))}
          </section>
        </section>}
      ...
      <section className="grid md:grid-cols-2 gap-20 maxWidth">
        ...
        <section>
          ...
          {/* system requirement  */}
          {!!gameData?.pc_requirements && <section>
            <div className="py-4">
          ...
          {/* mature content  */}
          {!!gameData?.content_descriptors?.notes && <section>
            <div className="py-4">
